<template>
  ...
  >
    <template v-slot:contents>
      <v-row>
        <h5>{{entityName}} 번역본을 삭제하시겠습니까?</h5>
      </v-row>
      <v-row>
        <h5>대상 {{entityName}} (번역명): {{props.entityTrans['name']}}</h5>
      </v-row>
      <v-row>
        <h5>{{ entityName }} 번역 언어: {{props.entityTrans['language']}}</h5>
      </v-row>
      <v-row>
        <p class="delete_msg">
          {{ entityName }} 삭제시 다국어화가 적용되는 모든 관련 정보가 삭제됩니다.<br/>
          여기에는 {{ entityName }} 상세 정보(해당 언어 번역본), {{ entityName }} 이미지가 포함됩니다.<br/>
          다음의 추가 데이터도 동시에 삭제됩니다: {{ includes ? includes : '해당사항 없음' }} <br/>
          {{ entityName }} 번역본이 존재하지 않는 언어 사용자에게는 해당 {{ entityName }}(이)가 노출되지 않게 됩니다.<br/>
        </p>
      </v-row>
      <v-row>
...
const props = defineProps({open: Boolean, entityName: String, includes: String, endpoint: String, entityTrans: Object});
